...
export interface LiquidGlassProps {
    ...
    /**
     * 鼠标悬停/按下辉光效果（通知弹窗不需要，传 false 关闭）
     */
    hoverEffect?: boolean
    onClick?: () => void
}
...
export default function LiquidGlass({
    ...
    return (
        <div
            ...
            onClick={onClick}
            onMouseEnter={clickable && hoverEffect ? () => setIsHovered(true) : undefined}
            onMouseLeave={clickable && hoverEffect ? () => { setIsHovered(false); setIsActive(false) } : undefined}
            onMouseDown={clickable && hoverEffect ? () => setIsActive(true) : undefined}
            onMouseUp={clickable && hoverEffect ? () => setIsActive(false) : undefined}
        >
            {lensMap?.url && (
                <GlassFilter
                    id={filterId}
