...
- **一条回复 = 多条气泡**。形态整形（`shapeReply(...).join('\n\n')`）按本人"连发短消息"
  的习惯把回复切成 4~5 段；聊天抽屉必须按空行把它们渲染成**多条** `.weclone-bubble`
  （`src/utils/weCloneBubbles.ts`，有单测）。旧版把整段塞进一个气泡 + `white-space: pre-wrap`，
  于是那些段落显示成一张卡片里的空行 —— 这正是用户报的"老是写多行长诗、还莫名多出
  两个换行"。离线夹具 `scripts/weclone-eval/bubbles.mjs` 一直是按"空行 = 真气泡"量的，
  界面与夹具口径必须一致，否则量出来的数字与用户看到的东西不是一回事。
- 连发的多条里**只有最后一条带"尾巴"**（那个直角角标）：每条都带会读成"四张独立卡片"。
- **默认管线 = v6**（清洗指纹 + 形态整形开、示例注入关、长度提示关，
  `WECLONE_SETTINGS_DEFAULT`）。v1.0.1 发的就是这一套；v6 与 control 在 200 例上文本指标
  打平（差异远小于噪声），而响应更快。换默认之前先看 `docs/research/weclone/06-loop-log.md`
  的 200 例复测 —— 60 例上的结论有两条是错的。
