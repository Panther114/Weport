...
import { DEFAULT_LLM_BASE_URL, DEFAULT_LLM_MODEL } from './llm/proxy'
...
async function main(): Promise<void> {
  ...
  // `WECLONE_E2E=1` lifts the per-route limits for local evaluation runs. The upload
  // route allows a handful per hour by design (it is the expensive one), which makes a
  // measurement loop impossible — but that limit must stay on for anything reachable,
  // so it is an explicit opt-in rather than a relaxed default.
  const e2e = process.env.WECLONE_E2E === '1'
  await app.register(rateLimit, {
    global: !e2e,
    max: e2e ? 100000 : 240,
    timeWindow: '1 minute',
    errorResponseBuilder: () => ({ success: false, error: 'too many requests (set WECLONE_E2E=1 for local evaluation)' }),
  })
  ...
  console.log(
    `[server] llm proxy: ${process.env.WECLONE_LLM_API_KEY ? 'configured' : 'MOCK MODE — set WECLONE_LLM_API_KEY'} ` +
      // Read the defaults from the proxy instead of repeating the literals here: this
      // line kept claiming `muse-spark-1.2-contributor` after the default changed, so
      // the log disagreed with the model actually being called.
      `→ ${process.env.WECLONE_LLM_BASE_URL || DEFAULT_LLM_BASE_URL} (${process.env.WECLONE_LLM_MODEL || DEFAULT_LLM_MODEL})`,
  )
}
