...
//!
//! Error -1006 is a security/init failure (WeFlow treats it like InitProtection codes).
//! Root causes we harden against:
//! 1. Wrong resource path for InitProtection (must point at folder with wcdb_api.dll)
//! 2. DLL search path missing WCDB.dll / runtime next to wcdb_api
//! 3. Re-init after shutdown without clean reload (process-wide singleton)
//! 4. Non-ASCII / empty protection path
...
impl WcdbApi {
    fn bootstrap(resource_root: &Path) -> Result<Self, String> {
        ...
        if init_code != 0 {
            // Retry once: re-set dll dir + protection + init
            set_dll_search_dir(&dll_dir);
            let cpath = path_to_cstring(&dll_dir)?;
            let _ = unsafe { init_protection(cpath.as_ptr()) };
            let _ = unsafe { shutdown() };
            let init_code2 = unsafe { init() };
            if init_code2 != 0 {
                return Err(format!(
                    "{}\n\n(wcdb_init 返回 {init_code} / 重试 {init_code2})\nDLL 目录: {}\nInitProtection:\n{}",
                    explain_code(init_code2),
                    dll_dir.display(),
                    codes.join("\n")
                ));
            }
