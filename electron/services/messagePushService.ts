...
export class MessagePushService {
  ...
  /** 订阅推送（应用内弹窗）。返回取消订阅函数。 */
  onPush(listener: (payload: MessagePushPayload) => void): () => void {
    this.pushListeners.push(listener)
    return () => {
      const idx = this.pushListeners.indexOf(listener)
      if (idx >= 0) this.pushListeners.splice(idx, 1)
    }
  }

  /** 推送出口：替代原 WeFlow 的 SSE 广播（Weport 不内置 HTTP 服务）。 */
  private emitPush(payload: MessagePushPayload): void {
    for (const listener of this.pushListeners) {
      try {
        listener(payload)
      } catch (e) {
        console.error('[MessagePushService] push listener error:', e)
      }
    }
  }
  ...
  private isSelfRevokeMessage(message: Message): boolean {
    const content = `${message.rawContent || ''}\n${message.parsedContent || ''}`
    if (content.includes('你撤回') || content.includes('你 撤 回')) return true
    // 兜底：WeChat 4 的撤回行措辞可能因版本而异，且内容可能只在 compress_content
    // 里（rawContent/parsedContent 为空）。本人执行的撤回，撤回行 is_send=1，
    // 结合撤回行特征即可可靠判定，不依赖具体文案。
    if (message.isSend === 1 && this.isRevokeSystemMessage(message)) return true
    return false
  }
