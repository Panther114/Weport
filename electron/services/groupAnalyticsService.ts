...
class GroupAnalyticsService {
  ...
  async getGroupChats(): Promise<{ success: boolean; data?: GroupChatInfo[]; error?: string }> {
    try {
      ...
      const [memberCounts, messageCounts, contactInfo] = await Promise.all([
        wcdbService.getGroupMemberCounts(groupIds),
        wcdbService.getSessionMessageCounts(groupIds),
      ...
      for (const groupId of groupIds) {
        ...
        const displayName = contact?.displayName ||
          ...
          groupId
        const avatarUrl = avatarCacheService.localUrlOrOriginal(
          contact?.avatarUrl ||
        ...
        groups.push({
          ...
          memberCount: memberCounts.success && memberCounts.map && typeof memberCounts.map[groupId] === 'number'
            ...
            : 0,
          messageCount: messageCounts.success && messageCounts.counts && typeof messageCounts.counts[groupId] === 'number'
            ? messageCounts.counts[groupId]
            : 0,
          avatarUrl
        })
      }
      ...
      for (let i = 0; i < usernames.length; i += 60) {
        ...
        if (res.success && res.map) {
          for (const [u, hex] of Object.entries(res.map)) {
            ...
            if (!hexStr) continue
            try {
              const b64 = Buffer.from(hexStr, 'hex').toString('base64')
              if (b64) dataUrls[u] = `data:image/jpeg;base64,${b64}`
            } catch { /* noop */ }
          }
        }
      }
