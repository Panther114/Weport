...
/**
 * 32 位字级 XOR（就地，out 与 keystream 可重叠）。
 * XOR 无进位，按 32 位字运算与逐字节运算结果完全一致。
 * 4 字节对齐时用 Uint32Array 视图（实测 ~5 倍速于逐字节循环）；
 * 未对齐前缀/尾部逐字节处理。
 * 注意：writeUInt32BE(x ^ y) 在 x^y 符号位为 1 时会抛 RangeError，
 * 因此绝不能走 read/writeUInt32BE 路径——必须用无符号视图。
 */
function xorChunk(out: Buffer, data: Buffer, key: Buffer, offset: number, length: number): void {
    const end = offset + length
    let i = offset
    if (out.byteOffset % 4 === 0 && data.byteOffset % 4 === 0 && key.byteOffset % 4 === 0) {
        const wordLen = Math.floor(length / 4)
        if (wordLen > 0) {
            const o = new Uint32Array(out.buffer, out.byteOffset + offset, wordLen)
            const d = new Uint32Array(data.buffer, data.byteOffset + offset, wordLen)
            const k = new Uint32Array(key.buffer, key.byteOffset + offset, wordLen)
            for (let w = 0; w < wordLen; w += 1) o[w] = d[w] ^ k[w]
        }
        i = offset + wordLen * 4
    }
    for (; i < end; i += 1) {
...
function xorInPlace(data: Buffer, key: Buffer, length: number): void {
    xorChunk(data, data, key, 0, Math.min(length, data.length, key.length))
}

function xorInto(out: Buffer, data: Buffer, key: Buffer): void {
    const length = Math.min(out.length, data.length, key.length)
    xorChunk(out, data, key, 0, length)
}

// CDN 下载并发上限：朋友圈网格一次可见几十张图，全量并发会互相争抢带宽，
// 拖慢每张下载并触发超时 → 网格显示「加载失败」。限流后单张下载更快更稳。
const CDN_DOWNLOAD_CONCURRENCY = 4
let cdnActive = 0
const cdnQueue: Array<() => void> = []
async function withCdnSlot<T>(fn: () => Promise<T>): Promise<T> {
  if (cdnActive >= CDN_DOWNLOAD_CONCURRENCY) {
    await new Promise<void>((resolve) => cdnQueue.push(resolve))
  }
  cdnActive += 1
  try {
    return await fn()
...
class SnsService {
    ...
    /** 记录失败资源（HTTP 4xx / 坏钥等非瞬态失败，5 分钟内直接快速失败） */
    private rememberFailedResource(cacheKey: string): void {
        if (!cacheKey) return
        this.failedResourceCache.set(cacheKey, Date.now())
        if (this.failedResourceCache.size > 2000) {
            const now = Date.now()
            for (const [k, t] of this.failedResourceCache) {
                if (now - t > this.failedResourceCacheTtlMs) this.failedResourceCache.delete(k)
            }
        }
    }

    /**
     * 网格缩略图：大图（>720px 且 >128KB）经 nativeImage 缩到 720px，
     * 返回 JPEG。GIF（动图）与小图直接返回 null（用原图）。
     * 仅一次原生 resize，CPU/内存开销极小。
     */
    private makeGridThumbnail(data: Buffer, contentType: string): { mime: string; base64: string } | null {
        try {
            if (!data || data.length < 128 * 1024) return null
            if (String(contentType || '').toLowerCase().includes('gif')) return null
    ...
    async getTimeline(limit: number = 20, offset: number = 0, usernames?: string[], keyword?: string, startTime?: number, endTime?: number): Promise<{ success: boolean; timeline?: SnsPost[]; error?: string }> {
        ...
        const enrichedTimeline = result.timeline.map((post: any) => {
            ...
            // 头像本地化：head_image.db / 磁盘缓存 → weport-media:// 即时显示；
            // CDN 未缓存时后台下载，下次查询命中本地文件
            const rawAvatarUrl = contact?.avatarUrl || post.avatarUrl
            const localAvatarUrl = avatarCacheService.localUrlOrOriginal(rawAvatarUrl)
            if (rawAvatarUrl && localAvatarUrl === rawAvatarUrl) {
                void avatarCacheService.ensure(rawAvatarUrl)
            }
                ...
                if (base64Part) {
                    try {
                        ...
                        if (detectImageMime(cachedBuf, '').startsWith('image/')) {
                            ...
                            // 全图已在磁盘缓存：返回 cachePath，灯箱可本地即时读全图
                            let cachePath: string | undefined
                            try {
                                const p = this.getCacheFilePath(url)
                                if (existsSync(p)) cachePath = p
                            } catch { /* noop */ }
                            return { success: true, dataUrl: cachedDataUrl, cachePath }
                        }
                    } catch {
...
window.addEventListener('scroll',function(){document.getElementById('btt').classList.toggle('show',window.scrollY>600)})
    ...
    private async fetchAndDecryptImage(url: string, key?: string | number): Promise<{ success: boolean; data?: Buffer; contentType?: string; cachePath?: string; status?: number; error?: string }> {
        ...
        // 失败快速失败缓存键（与 proxyImage 一致）
        const failKey = `${this.normalizeCacheUrl(url)}|${key ?? ''}`
        ...
        if (isVideo) {
            // 视频专用下载逻辑 (下载 -> 解密 -> 缓存)
            return withCdnSlot(() => new Promise(async (resolve) => {
                ...
                try {
                    ...
                    const req = https.request(options, (res: any) => {
                        if (res.statusCode !== 200 && res.statusCode !== 206) {
                            ...
                            // 4xx 等确定性失败记住 5 分钟，避免反复重试；5xx 留给下次重试
                            if (Number(res.statusCode) < 500) this.rememberFailedResource(failKey)
                            ...
                            try {
                                ...
                                if (key && String(key).trim().length > 0) {
                                    try {
                                        ...
                                        xorInPlace(raw, keystream, decryptLen)
        ...
        // 图片逻辑 (保持流式处理)
        return withCdnSlot(() => new Promise((resolve) => {
            try {
                const https = require('https')
                ...
                const req = https.request(options, (res: any) => {
                    ...
                    stream.on('end', async () => {
                        ...
                        // 图片逻辑：原始数据已是有效图片（CDN 未加密内容）时跳过解密，
                        // 省去整段 keystream 生成与 XOR（大图可省数百 ms）
                        const shouldDecrypt = (xEnc === '1' || !!key) && key !== undefined && key !== null && String(key).trim().length > 0 && !rawMagicMime.startsWith('image/')
                        if (shouldDecrypt) {
                            try {
                                ...
                                if (/^\d+$/.test(keyStr)) {
                                    ...
                                    // XOR 解密（32 位字级运算，约 4 倍速于逐字节循环）
                                    xorInto(decrypted, raw, keystream)
                        ...
                        if (!decodedMagicMime.startsWith('image/')) {
                            ...
                            // 密钥不匹配/资源损坏是确定性失败，5 分钟内快速失败不再重试
                            this.rememberFailedResource(failKey)
