...
export class WasmService {
    ...
    /** keystream LRU 缓存：朋友圈同一账号的媒体常共享同一密钥，
        重复生成 4MB+ keystream 是解密慢的主因之一（wasm 生成 + 两次全量拷贝） */
    private keystreamCache = new Map<string, Buffer>();
    private keystreamCacheBytes = 0;
    private readonly keystreamCacheMaxBytes = 64 * 1024 * 1024;
    private readonly keystreamCacheMaxEntries = 32;

    private constructor() { }

    private cacheGet(cacheKey: string): Buffer | null {
        const hit = this.keystreamCache.get(cacheKey)
        if (hit) {
            // LRU 触碰
            this.keystreamCache.delete(cacheKey)
            this.keystreamCache.set(cacheKey, hit)
        }
        return hit || null
    }
    ...
    private cachePut(cacheKey: string, buffer: Buffer): void {
        ...
        while (
            this.keystreamCacheBytes > this.keystreamCacheMaxBytes ||
            this.keystreamCache.size > this.keystreamCacheMaxEntries
        ) {
            const oldestKey = this.keystreamCache.keys().next().value as string | undefined
            if (!oldestKey) break
            const oldest = this.keystreamCache.get(oldestKey)
            if (oldest) this.keystreamCacheBytes -= oldest.length
            this.keystreamCache.delete(oldestKey)
        }
    }

    /** 仅供测试/诊断 */
    public getKeystreamCacheStats(): { entries: number; bytes: number } {
        return { entries: this.keystreamCache.size, bytes: this.keystreamCacheBytes }
    }
    ...
    public async getKeystream(key: string, size: number = 131072): Promise<Buffer> {
        ...
        const cacheKey = `${key}:${alignSize}`;
        const cached = this.cacheGet(cacheKey);
        if (cached) {
            return Buffer.from(cached.subarray(0, size));
        }
